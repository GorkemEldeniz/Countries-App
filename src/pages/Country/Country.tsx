...
const Country = () => {
  ...
  const [{ name, flags: { svg }, nativeName, population, topLevelDomain, region, currencies, languages, subregion, capital, borders }] = useMemo(() => data.filter(el => el.name.toLowerCase() == country?.toLowerCase()), [country])
        ...
        <div>
          ...
          <div>
            <p>Border Countries: <div className="borders">
              {borders?.map(el => <span key={el}>{el}</span>)}
            </div></p>
          </div>
        </div>
      </div>
